class_name DynamicShadow extends Area2D
...
func transform_sprite_image_to_shadow(light_position: Vector2) -> void:
	...
	if using_polygon:
		...
		for i in range(2, num_shadow_sprites):
			...
			var scaled_collision_shape := scale_collision_polygon(base_collision_poly, position_offset, scale_amount.x)
...
func scale_collision_polygon(coll_poly: CollisionPolygon2D, position_offset: Vector2, scale_amount: float) -> PackedVector2Array:
...
func _on_mouse_enter():
	if owned_by_enemy and not Global.collected_all_fragments: return
	sprite.material.set_shader_parameter("do_outline", true)
	mouse_inside = true

func _on_mouse_exit():
	if owned_by_enemy and not Global.collected_all_fragments: return
	sprite.material.set_shader_parameter("do_outline", false)
	mouse_inside = false
...
func possess_as_enemy() -> void:
	print("dynamic shadow possess")
	if owned_by_enemy:
		possessed.emit(self)
